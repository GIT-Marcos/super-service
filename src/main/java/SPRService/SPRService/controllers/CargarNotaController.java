...
public class CargarNotaController implements Initializable, ModalController<NotaRetiro> {
    ...
    private void guardarNota(ActionEvent event) {
        if (detalles.isEmpty()) {
            NotificationHelper.mostrarAdvertencia("Cargar nota", "Debe agregar productos a la nota para poder cargarla.");
            return;
        }
        ...
        try {
            NotaRetiro nota = new NotaRetiro(NotaRetiro.TipoUsoRetiro.OTRO, detalles);
            this.paraDevolver = notaRetiroServ.guardarNota(nota);
            NotificationHelper.mostrarExito("Cargar nota", "La Nota de Retiro se ha cargado con éxito.");
            Node n = ((Node) event.getSource());
            Stage s = (Stage) n.getScene().getWindow();
            s.close();
        } catch (Exception e) {
            NotificationHelper.mostrarError("Cargar nota", e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void gestionarNota(ActionEvent event) {
        ...
        if (file == null) return;
        GeneradorTXT.generaNotaRetiro(detalles.stream().toList(), file);
